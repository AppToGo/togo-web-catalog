...
import { X, Minus, Plus, ShoppingBag, Loader2, AlertCircle, CheckCircle } from 'lucide-react';
...
export function CartDrawer({ isOpen, onClose, token, business }: CartDrawerProps) {
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      if (orderStatus?.hasOrder && orderStatus.order && orderStatus.order.status !== 'DRAFT') {
        setShowAlert({
          type: 'error',
        ...
        {showAlert && (
          <div className={`px-4 py-3 ${
            ...
            <div className="flex items-start gap-2">
              {showAlert.type === 'error' && <AlertCircle className="w-5 h-5 flex-shrink-0 mt-0.5" />}
              {showAlert.type === 'success' && <CheckCircle className="w-5 h-5 flex-shrink-0 mt-0.5" />}
