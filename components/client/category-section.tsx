...
export function CategorySection({ id, title, products, useProductImages }: CategorySectionProps) {
    ...
    <div className="pb-2">
      {title && (
        <div className="sticky top-[124px] z-20 bg-[var(--surface)] flex items-baseline gap-2 px-4 pt-3 pb-2 border-b border-[var(--line)]">
          ...
          <span className="text-xs text-[var(--ink-3)] font-medium">{filteredProducts.length} items</span>
        </div>
      )}
      <ProductListClient products={filteredProducts} categoryId={id} useProductImages={useProductImages} />
    </div>
  );
}
