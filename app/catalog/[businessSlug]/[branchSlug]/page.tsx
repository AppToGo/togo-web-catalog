...
export default async function BranchCatalogPage({ params, searchParams }: PageProps) {
  const { businessSlug, branchSlug } = await params;
  const { t: token, source, table } = await searchParams;

  if (!businessSlug || !isValidSlug(businessSlug) || !branchSlug || !isValidSlug(branchSlug)) {
